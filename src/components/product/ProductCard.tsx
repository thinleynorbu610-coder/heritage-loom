      ...
      <div className="relative aspect-[4/5] overflow-hidden rounded-lg bg-surface-muted">
        ...
        <div className="pointer-events-none absolute top-3 left-3 flex flex-col items-start gap-1.5">
          {soldOut ? (
            ...
            <Badge tone="sale">New</Badge>
          ) : null}
        </div>

        <WishlistButton productId={product.id} productName={product.name} className="absolute top-3 right-3" />

        {/* Desktop quick-add, revealed on hover/focus */}
        <div className="absolute inset-x-3 bottom-3 hidden translate-y-2 opacity-0 transition-all duration-300 group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:translate-y-0 group-hover:opacity-100 md:block">
          <button
            type="button"
            onClick={onAdd}
            disabled={soldOut}
            className="flex h-11 w-full items-center justify-center gap-2 rounded-md bg-surface/95 text-sm font-semibold text-text shadow-soft backdrop-blur-sm transition-colors hover:bg-ink hover:text-background disabled:opacity-60"
          >
