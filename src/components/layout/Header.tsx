...
function HeaderAction({
  ...
  const inner = (
    <>
      <span className="relative">
        {icon}
        {!!count && (
          <span className="absolute -top-2 -right-2.5 flex h-[1.15rem] min-w-[1.15rem] animate-scale-in items-center justify-center rounded-full bg-primary px-1 text-[0.65rem] font-bold text-white tabular-nums">
            {count}
          </span>
        )}
      </span>
  ...
  const cls = cn(
    "flex h-14 min-w-14 flex-col items-center justify-center gap-1 rounded-md px-2 text-text transition-colors hover:bg-surface-muted hover:text-primary",
    className,
  );
    ...
    <>
      ...
      <header
        className={cn(
          ...
          scrolled
            ? "border-border bg-background/92 shadow-[0_6px_24px_-18px_rgb(45_30_20/0.5)] backdrop-blur-md"
      ...
      >
        <div className="container-page flex h-[var(--header-h)] items-center gap-4">
          ...
          <button
            ...
            className="-ml-2 flex flex-col items-center gap-0.5 rounded-md p-2 text-text lg:hidden"
              ...
              item.label === "Categories" ? (
                <div key={item.href} className="group/cat relative">
                  <Link
                    href={item.href}
                    className={cn(
                      "flex items-center gap-1 rounded-md px-3.5 py-2 text-[0.94rem] font-semibold transition-colors hover:text-primary",
                ...
                <Link
                  ...
                  className={cn(
                    "relative rounded-md px-3.5 py-2 text-[0.94rem] font-semibold transition-colors hover:text-primary",
                    isActive(item.href) ? "text-primary" : "text-text",
                  )}
                >
                  {item.label}
